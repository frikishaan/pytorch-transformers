...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        PyTorch doesn't have a vectorized pow(base, exponent_vector) that's as convenient or numerically stable, so the standard trick is:
...
def attention(q: Tensor, k: Tensor, v: Tensor, mask: Tensor, dropout: nn.Dropout) -> Tuple[Tensor, Tensor]:
    d_k = q.shape[-1]
